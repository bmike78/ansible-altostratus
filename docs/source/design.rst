...
What is Altostratus ?
---------------------
`Altostratus`_ combines `Ansible`_ playbook roles with cloud resources, such as `Openstack`_ and `CloudFoundry`_ which takes a different approach to launching and managing resources:

- It uses Ansible which can be used to create more complex workflows and piece together different pieces of infrastructure
- It uses reusable tasks and roles that can be used in multiple clouds (Openstack, CloudFoundry currently)
...
------------------------------
**Altostratus** is defined as 
**a layer cloud at an intermediate height of about 2400 to 6000 metres (8000 to 20 000 feet).**  

From the latin words of Alto- (Middle) and stratus (layered)

The Altostratus playbook deploys down to your cloud, but can be used by interfaces above it, like ansible-playbook, API, Jenkins, Rundeck, Ansible Tower, AWX or Vespene.  It sits somewhere in the middle of these layers, just like `altostratus clouds`_ 

.. _altostratus clouds: https://en.wikipedia.org/wiki/Altostratus_cloud
...
Does Altostratus support other clouds like AWS, Azure, GCP, VMware, etc ?
--------------------------------------------------------------------
Not at this time, but functionality could be added later to do this with additional Ansible roles.  Some roles or playbooks may exist that could be imported or contributed.
