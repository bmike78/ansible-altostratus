...
---------------------
`Altostratus`_ is designed to utilize `Ansible`_, Ansible Galaxy and custom roles to interact with cloud resources, such as `Openstack`_ and `CloudFoundry`_ to manage resources in multiple clouds. 

As an IT professional who manages several Openstack clouds, is a CloudFoundry consumer and potential consumer of other clouds such as AWS, Azure and GCP, I wanted to come up with a way to manage these resources easily and allow for cloud portability if my workload needs to shift from one cloud to the other or if I need to interact with multiple cloud providers for an application deployment.
...
- Creation of instances should work the same no matter what cloud provider is used.  This is an easy thing to say until every cloud provider gives you a different way of interacting with them.  One is easy, many is hard.
...
- Sometimes in the middle of an existing deployment, settings, configuration or individual roles need to be changed and Ansible allows for these kind of support activities to happen.
- There are orchestration tools like Openstack Heat or AWS CloudFormation that accomplish similar things to Ansible to deploy, but they are not cloud agnostic or span outside their cloud.
...
- Many of my teams are starting to use Ansible or have their own automation efforts, so this project is an attempt to utilize one tool for all our efforts and allow for them to consume and contribute to this tool.
...
- Even though Openstack is only supported today, a standard inventory file with groups assigned as roles or using the add_host module to iterate through instances/VMs/metal can be used in the same way to push down applications.
...
---------------------
- `Altostratus`_ serves as a provisioner for cloud resources and application deployments to provide end to end automation defined in a yml file.
